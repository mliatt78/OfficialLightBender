...
function MakeTest() {
	...
	ready=false;
	curFX.particleSystem.Stop();
	yield WaitForSeconds(0.5);
	curFX.particleSystem.Play();
	yield WaitForSeconds(0.5);
	ready=true;
}
...
function OnGUI () {
	...
	if (GUI.Button (Rect ((viewOptionsBoxOffset[0] + 10),(viewOptionsBoxOffset[1] + 30),100,20), "Toggle Floor")) {
		...
		iPlane.renderer.enabled = !iPlane.renderer.enabled;
	}

	if (GUI.Button (Rect ((viewOptionsBoxOffset[0] + 10),(viewOptionsBoxOffset[1] + 60),100,20), "Toggle Skybox")) {
		var cam : GameObject;
		cam = GameObject.Find("Main Camera");
		if (cam.camera.clearFlags == 2) {
			cam.camera.clearFlags = 1;
		}
		else if (cam.camera.clearFlags == 1) {
			cam.camera.clearFlags = 2;
			cam.camera.backgroundColor = Vector4(0.5, 0.5, 0.5, 1.0);
		}
	}
}